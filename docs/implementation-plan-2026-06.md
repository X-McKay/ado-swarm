...
- **Phase 1 — done.** Strands model factory + deterministic `FakeModel`; `run_model_agent` runtime; `ToolPolicyHook` (`BeforeToolCallEvent`); tool catalog (now 22 tools); `AgentSkills`-bound skills single-sourced from metadata; **all agents** model-driven via `CasefileAgent`/`run_model_agent`; non-deprecated `invoke_async(structured_output_model=...)`; shared golden-eval harness; vocabulary guardrail test.
...
- **OTel GenAI tracing — done.** Config-gated (`tracing_enabled`, default off) `runtime/telemetry.py`: Strands `StrandsTelemetry` OTLP/console exporter set up at agent-runtime + Temporal client; `TracingInterceptor` wired into `build_temporal_client`; agent spans carry service identity + `ado_swarm.agent_id`. No-op (zero overhead/output) when disabled.
- **Provider-contract semantics pinning — done.** `SourceIssue.external_id` round-trip semantics documented on the contract; `ProviderMutationResult` gains a required `result_kind` (`MutationResultKind`) so `external_id` unambiguously identifies the *created* comment/PR across all providers (stub fixed to return a comment id, not the parent issue id).
- **Swarm eval comparison — done.** `evals/swarm_comparison.py` + `ado-swarm eval-swarm` run `security_reviewer` single-agent vs swarm over golden adjudication cases and emit a data-driven `recommendation` (tie → keep the cheaper single agent). The swarm default stays off until a real-model run says otherwise.

Gate status: `ruff` + `ruff format` + `ty` clean; 165 unit/workflow tests pass (4 skipped); `eval-agents` 10/10 on `fake`; tool catalog = 22 tools; 10 agents (submission_engineer is the terminal, approval-gated pipeline stage). **All planned phases (0–4) and follow-on items are complete.**
